# Git 操作指南 - 版本保存与合并

## 📦 一、保存当前版本（创建备份）

### 1. 查看当前状态
```bash
# 查看有哪些文件被修改了
git status

# 查看具体的修改内容
git diff
```

### 2. 提交当前所有更改到本地仓库
```bash
# 添加所有修改的文件
git add .

# 或者选择性添加特定文件
git add frontend/src/components/Map3DInterface.vue
git add backend/app/api/endpoints/chat.py

# 提交更改（创建一个版本快照）
git commit -m "保存当前版本：3D地图界面完整功能"
```

### 3. 创建备份分支（推荐！）
```bash
# 创建一个新分支保存当前版本
git branch backup-2024-12-30

# 或者创建一个带描述的备份分支
git branch backup-before-merge-3dmap-complete

# 查看所有分支
git branch

# 切换回主分支（如果需要）
git checkout main
# 或
git checkout master
```

### 4. 推送到远程仓库（可选，但强烈推荐）
```bash
# 推送当前分支到远程
git push origin main

# 推送备份分支到远程
git push origin backup-2024-12-30
```

---

## 🔄 二、拉取并合并新版本

### 1. 确保当前工作已保存
```bash
# 先提交或暂存当前更改
git add .
git commit -m "保存当前工作"
```

### 2. 拉取远程最新版本
```bash
# 查看远程仓库信息
git remote -v

# 获取远程最新信息（不合并）
git fetch origin

# 查看远程分支
git branch -r

# 查看远程和本地的差异
git log HEAD..origin/main
```

### 3. 合并远程版本（三种方式）

#### 方式A：直接合并（保留双方更改）
```bash
# 拉取并合并
git pull origin main

# 如果有冲突，Git 会提示，需要手动解决
```

#### 方式B：使用 merge（推荐，保留完整历史）
```bash
# 先拉取
git fetch origin

# 合并远程分支
git merge origin/main

# 如果有冲突，会显示冲突文件
```

#### 方式C：使用 rebase（线性历史，但会改写历史）
```bash
# 不推荐新手使用，会改写提交历史
git pull --rebase origin main
```

---

## ⚠️ 三、处理合并冲突（选择保留哪些更改）

### 1. 查看冲突文件
```bash
# 合并后，查看哪些文件有冲突
git status
```

### 2. 手动解决冲突

冲突文件会包含类似这样的标记：
```
<<<<<<< HEAD
你的本地代码
=======
远程的代码
>>>>>>> origin/main
```

### 3. 使用工具解决冲突

#### 方法A：使用 VS Code（推荐）
```bash
# VS Code 会自动检测冲突，提供可视化界面
# 点击 "Accept Current Change" 保留你的
# 点击 "Accept Incoming Change" 使用远程的
# 点击 "Accept Both Changes" 保留两者
```

#### 方法B：使用 Git 命令选择
```bash
# 完全使用你的版本（覆盖远程）
git checkout --ours 文件名

# 完全使用远程版本（覆盖本地）
git checkout --theirs 文件名

# 例如：
git checkout --ours frontend/src/components/Map3DInterface.vue
git checkout --theirs backend/app/core/config.py
```

#### 方法C：手动编辑文件
- 打开冲突文件
- 删除冲突标记（`<<<<<<<`, `=======`, `>>>>>>>`）
- 保留你想要的代码
- 保存文件

### 4. 标记冲突已解决
```bash
# 解决冲突后，添加文件
git add 冲突文件名

# 或者添加所有已解决的文件
git add .

# 完成合并
git commit -m "合并远程版本，解决冲突"
```

---

## 🔙 四、恢复到之前的版本

### 1. 查看提交历史
```bash
# 查看提交历史（带图形）
git log --oneline --graph --all

# 查看详细历史
git log

# 查看某个文件的修改历史
git log -- frontend/src/components/Map3DInterface.vue
```

### 2. 恢复到之前的提交

#### 方法A：创建新分支恢复（推荐，不丢失当前工作）
```bash
# 查看提交ID
git log --oneline

# 基于某个提交创建新分支
git checkout -b restore-version <提交ID>

# 例如：
git checkout -b restore-version abc1234
```

#### 方法B：重置到之前的提交（危险！会丢失之后的工作）
```bash
# 软重置（保留工作区的更改）
git reset --soft <提交ID>

# 混合重置（保留工作区，但取消暂存）
git reset --mixed <提交ID>

# 硬重置（完全丢弃，危险！）
git reset --hard <提交ID>
```

#### 方法C：恢复特定文件到之前的版本
```bash
# 恢复单个文件到某个提交的版本
git checkout <提交ID> -- 文件路径

# 例如：
git checkout abc1234 -- frontend/src/components/Map3DInterface.vue
```

### 3. 切换到备份分支
```bash
# 查看所有分支（包括备份分支）
git branch -a

# 切换到备份分支
git checkout backup-2024-12-30

# 如果需要，可以基于备份分支创建新分支继续工作
git checkout -b continue-from-backup
```

---

## 🛡️ 五、安全操作流程（推荐）

### 完整的安全合并流程：

```bash
# 1. 查看当前状态
git status

# 2. 保存当前工作
git add .
git commit -m "保存当前版本"

# 3. 创建备份分支
git branch backup-before-merge-$(date +%Y%m%d)

# 4. 推送到远程（可选但推荐）
git push origin backup-before-merge-$(date +%Y%m%d)

# 5. 拉取远程更新
git fetch origin

# 6. 查看差异
git log HEAD..origin/main

# 7. 合并
git merge origin/main

# 8. 如果有冲突，解决冲突
# （使用 VS Code 或手动编辑）

# 9. 完成合并
git add .
git commit -m "合并远程版本"

# 10. 推送到远程
git push origin main
```

---

## 📋 六、常用命令速查

```bash
# 查看状态
git status
git diff

# 保存更改
git add .
git commit -m "描述"

# 分支操作
git branch                    # 查看分支
git branch 分支名             # 创建分支
git checkout 分支名           # 切换分支
git branch -d 分支名          # 删除分支

# 远程操作
git fetch origin              # 获取远程更新
git pull origin main          # 拉取并合并
git push origin main          # 推送到远程

# 查看历史
git log --oneline --graph     # 图形化历史
git log --all                 # 所有分支历史

# 恢复操作
git checkout <提交ID> -- 文件  # 恢复文件
git reset --soft <提交ID>      # 软重置
git checkout 分支名            # 切换分支
```

---

## ⚠️ 注意事项

1. **合并前一定要先提交或创建备份分支**
2. **使用 `git reset --hard` 要非常小心，会丢失未提交的工作**
3. **冲突解决后记得 `git add` 和 `git commit`**
4. **重要更改建议先推送到远程，作为额外备份**
5. **不确定时，先创建分支实验，不要直接在主分支操作**

---

## 🎯 针对你的情况的具体建议

基于你当前的工作，建议执行：

```bash
# 1. 保存当前版本
git add .
git commit -m "3D地图界面完整功能：包含导航、聊天、地图显示等"

# 2. 创建备份分支
git branch backup-3dmap-complete-20241230
git push origin backup-3dmap-complete-20241230

# 3. 拉取并合并
git fetch origin
git merge origin/main

# 4. 如果有冲突，在 VS Code 中解决
# 5. 完成合并后测试功能
# 6. 如果合并后有问题，可以切换回备份分支：
#    git checkout backup-3dmap-complete-20241230
```


